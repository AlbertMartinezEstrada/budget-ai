...
function renderCategories(categories) {
    ...
    if (!categories || categories.length === 0) {
        container.innerHTML = '<p class="text-gray-500 col-span-4 text-center py-8">No hay categorías</p>';
        return;
    }
    ...
    container.innerHTML = categories.map((cat, idx) => `
        <div class="bg-white rounded-xl p-4 shadow-sm border border-slate-200 flex items-center gap-3">
            ...
            <div>
                <h3 class="font-semibold">${cat.nom}</h3>
                <span class="text-xs text-gray-500">${cat.tipus || 'Sin tipo'}</span>
            </div>
        </div>
    `).join('');
}